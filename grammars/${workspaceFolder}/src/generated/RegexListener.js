// Generated from c:/Users/Mariano/regex-analyzer/grammars/Regex.g4 by ANTLR 4.13.1
// jshint ignore: start
import antlr4 from 'antlr4';

// This class defines a complete listener for a parse tree produced by RegexParser.
export default class RegexListener extends antlr4.tree.ParseTreeListener {

	// Enter a parse tree produced by RegexParser#regex.
	enterRegex(ctx) {
	}

	// Exit a parse tree produced by RegexParser#regex.
	exitRegex(ctx) {
	}


	// Enter a parse tree produced by RegexParser#term.
	enterTerm(ctx) {
	}

	// Exit a parse tree produced by RegexParser#term.
	exitTerm(ctx) {
	}


	// Enter a parse tree produced by RegexParser#factor.
	enterFactor(ctx) {
	}

	// Exit a parse tree produced by RegexParser#factor.
	exitFactor(ctx) {
	}


	// Enter a parse tree produced by RegexParser#base.
	enterBase(ctx) {
	}

	// Exit a parse tree produced by RegexParser#base.
	exitBase(ctx) {
	}


	// Enter a parse tree produced by RegexParser#group.
	enterGroup(ctx) {
	}

	// Exit a parse tree produced by RegexParser#group.
	exitGroup(ctx) {
	}


	// Enter a parse tree produced by RegexParser#class.
	enterClass(ctx) {
	}

	// Exit a parse tree produced by RegexParser#class.
	exitClass(ctx) {
	}


	// Enter a parse tree produced by RegexParser#class_item.
	enterClass_item(ctx) {
	}

	// Exit a parse tree produced by RegexParser#class_item.
	exitClass_item(ctx) {
	}


	// Enter a parse tree produced by RegexParser#range.
	enterRange(ctx) {
	}

	// Exit a parse tree produced by RegexParser#range.
	exitRange(ctx) {
	}


	// Enter a parse tree produced by RegexParser#quantifier.
	enterQuantifier(ctx) {
	}

	// Exit a parse tree produced by RegexParser#quantifier.
	exitQuantifier(ctx) {
	}


	// Enter a parse tree produced by RegexParser#number.
	enterNumber(ctx) {
	}

	// Exit a parse tree produced by RegexParser#number.
	exitNumber(ctx) {
	}


	// Enter a parse tree produced by RegexParser#char.
	enterChar(ctx) {
	}

	// Exit a parse tree produced by RegexParser#char.
	exitChar(ctx) {
	}


	// Enter a parse tree produced by RegexParser#class_char.
	enterClass_char(ctx) {
	}

	// Exit a parse tree produced by RegexParser#class_char.
	exitClass_char(ctx) {
	}



}